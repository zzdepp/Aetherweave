...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const SYSTEM_INSTRUCTION_TEXT = `You are Aetherweave AI, a versatile and intelligent AI Assistant specializing in Computer Servicing, IT & Technical Support, programming, general knowledge, creative ideas, and everyday questions.

Key Instructions:
- Answer ANY topic or question the user asks accurately, clearly, and thoughtfully.
...
- Maintain a friendly, helpful, and respectful tone at all times.`;
  ...
  const generateClientFallback = (userMessage = '') => {
    ...
    if (msg.includes('power') || msg.includes('turn on') || msg.includes('display') || msg.includes('black screen') || msg.includes('no signal') || msg.includes('boot')) {
      return `### 🔧 PC No Power / No Display Diagnostic Guide\n\n1. **Check Power Connections & PSU Switch:**\n   - Ensure the PSU switch on the back of the case is set to **'I'** (On).\n   - Re-seat the 24-pin motherboard power cable and 8-pin CPU power cable tightly.\n\n2. **RAM / Memory Reseating (Most Common fix for No Display):**\n   - Turn off PC & unplug power.\n   - Remove your RAM sticks, clean gold contacts with an eraser, and re-insert firmly in slots A2 and B2.\n\n3. **GPU & Display Cable Checks:**\n   - Plug HDMI/DisplayPort cable directly into GPU, not motherboard.\n\n4. **Clear CMOS / Reset BIOS:**\n   - Remove CR2032 coin battery from motherboard for 5 minutes while unplugged.`;
    }

    if (msg.includes('heat') || msg.includes('temp') || msg.includes('paste') || msg.includes('fan') || msg.includes('cooler') || msg.includes('shutdown')) {
      return `### 🌡️ CPU Overheating & Thermal Maintenance\n\n1. **Check Temperatures:** Idle should be **35°C–50°C**; under heavy load, stay under **85°C** (use HWiNFO64).\n2. **Replace Thermal Paste:**\n   - Turn off & unplug PC. Remove cooler gently.\n   - Clean old crusty paste using 90%+ Isopropyl Alcohol and a lint-free cloth.\n   - Apply a pea-sized dot (~4-5mm) of thermal paste in center of CPU lid.\n   - Reattach cooler evenly in cross pattern.\n3. **Dust & Airflow:** Front fans pull air IN, rear/top fans push air OUT.`;
    }

    if (msg.includes('slow') || msg.includes('lag') || msg.includes('ssd') || msg.includes('hdd') || msg.includes('drive')) {
      return `### 💾 Speeding Up a Computer & Storage Diagnostics\n\n1. **Upgrade to SSD:** Moving OS from mechanical HDD to NVMe/SATA SSD increases speed by 5x-10x.\n2. **Check Memory Usage (RAM):** Press \`Ctrl + Shift + Esc\` to open Task Manager. If RAM is over 85%, close apps or add RAM.\n3. **Disable Startup Apps:** In Task Manager > Startup Apps, turn off unnecessary background apps.`;
    }

    if (msg.includes('format') || msg.includes('install') || msg.includes('windows') || msg.includes('os') || msg.includes('iso')) {
      return `### 💻 Windows Clean Installation & Formatting Guide\n\n1. **Bootable USB:** Create 8GB+ USB drive with Rufus or Windows Media Creation Tool.\n2. **Boot Menu:** Restart PC and press \`Del\`, \`F2\`, or \`F12\` to select USB boot drive.\n3. **Clean Setup:** Choose **Custom: Install Windows only**, format C: partition, and complete setup.`;
    }
    ...
    return `### 🤖 Aetherweave AI Assistant\n\nHere is a direct overview regarding **"${userMessage || 'your inquiry'}"**:\n\n- **Computer Servicing & Support:** I provide step-by-step guidance on hardware diagnostics, PC building, thermal paste replacement, RAM testing, and BSoD error resolution.\n- **IT & OS Maintenance:** Windows 10/11 clean formatting, driver updates, network configuration, and disk optimization.\n- **General Tech & Programming:** Code explanations, debugging, script creation, and technology advice.\n\nFeel free to ask any specific follow-up question or request detailed step-by-step procedures!`;
  };

  const callGeminiDirect = async (apiKey, userText, history) => {
    ...
    for (const modelName of candidateModels) {
      try {
        ...
        console.warn(`[Client Gemini ${modelName} Warning]:`, err.message);
        lastError = err;
      }
    }
    throw lastError || new Error('Unable to connect to Gemini API.');
  };
  ...
  const agoraChat = {
    ...
    getStoredApiKey: () => {
      return localStorage.getItem('aetherweave_gemini_api_key') || localStorage.getItem('gemini_api_key') || '';
    },
    setStoredApiKey: (key) => {
      if (key && key.trim()) {
        localStorage.setItem('aetherweave_gemini_api_key', key.trim());
      } else {
        localStorage.removeItem('aetherweave_gemini_api_key');
      }
    },
    sendMessage: async (userText) => {
      const storedKey = agoraChat.getStoredApiKey();

      // 1. If Gemini API key is stored in localStorage, call direct client-side Gemini API
      if (storedKey) {
        try {
          const reply = await callGeminiDirect(storedKey, userText, agoraChat.history);
          agoraChat.history.push({ role: 'user', text: userText });
          agoraChat.history.push({ role: 'model', text: reply });
          return reply;
        } catch (clientErr) {
          console.warn('[Direct Gemini Client Call failed, trying server endpoint]:', clientErr.message);
        }
      }

      // 2. Try Node.js server API endpoint /api/ai-chat
      try {
        const res = await fetch('/api/ai-chat', {
          ...
          body: JSON.stringify({ message: userText, history: agoraChat.history, apiKey: storedKey })
        });
        ...
        console.warn('[Server AI Endpoint unreachable]');
      }

      // 3. Clean fallback response
      const fallbackReply = generateClientFallback(userText);
      agoraChat.history.push({ role: 'user', text: userText });
      agoraChat.history.push({ role: 'model', text: fallbackReply });
